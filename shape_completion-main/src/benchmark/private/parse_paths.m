...
switch c.curr_tgt_ds
    ...
    case {'smal'}
        paths = load_smal_paths(c,outputs); 
    case {'dfaust'}
        error('Unimplemented');
    otherwise
...
for i=1:N
    ...
    switch lower(c.curr_src_ds)
        ...
        otherwise
            ...
            ids = regexp(outputs{i},'\d*','Match'); % sub,pose,sub,pose,mask
            paths{i,2} = sprintf('tr_reg_0%s%s_%03s.npz',ids{3},ids{4},num2str(str2num(ids{5})-1)); % The part
            paths{i,3} = sprintf('tr_reg_0%s%s.off',ids{3},ids{4}); % The Ground Truth
            paths{i,4} = sprintf('tr_reg_0%s%s.off',ids{1},ids{2}); % The Template
    end
...
function [paths] = load_smal_paths(c, outputs)
N = length(outputs); 
paths = cell(N,5);
for i=1:N
    ...
    ids = split(outputs{i},'_'); 
    paths{i,2} = fullfile(ids{2},ids{3},sprintf('%s_%s.npz',ids{3},ids{4})); % The part
    paths{i,3} = fullfile(ids{2},sprintf('%s.ply',ids{3})); % The Ground Truth
    paths{i,4} = fullfile(ids{6},sprintf('%s.ply',ids{7})); % The Template
