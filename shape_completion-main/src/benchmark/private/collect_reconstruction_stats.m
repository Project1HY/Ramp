...
if force || ~stats_exists || ~isfield(stats,c.curr_exp)
    ...
    N = size(paths,1);
    me_err = zeros(N,1); vol_err = zeros(N,1);tp_me_err = zeros(N,1); 
    ...
    %     progressbar;
    parfor i=1:N
        [resM,gtM,partM,tpM,mask] = load_path_tup(c,paths(i,:));
        if resM.Nf ~= gtM.Nf
            gtM = qslim(gtM,resM.Nf);
        end
        % Align Result<->GT
        resM.v = compute_icp(resM.v,gtM.v,true);
        diff_tp = abs(compute_icp(tpM.v,gtM.v,true) - gtM.v).^2;
        tp_me_err(i) = sqrt(sum(diff_tp(:))/numel(tpM.v));
        % Compute Mean Error
