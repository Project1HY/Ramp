...
if __name__ == '__main__':
    ...
    fp = Path(__file__).parents[0] / 'dataset_2020-05-22_23_23_43' / 'meshes'
    dset_fps = list(fp.glob('*'))
    N = 36
    vs, fs = [], []
    for i in range(N):
        M = read_mesh(str(dset_fps[i]))
        vs.append(M.nodes)
        fs.append(M.tris)
        print(vs[i].shape, fs[i].shape)  # Not of equal verts or faces

    plot_mesh_montage(vs, fs, strategy="spheres")
