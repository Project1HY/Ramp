...
from util.container import max_dict_depth, min_dict_depth, to_list, deep_dict_convert
...
class HierarchicalIndexTree:
    def __init__(self, hit, in_memory):
        ...
        self._rhit = deep_dict_convert(hit)  # Not very efficient to duplicate the tree, but meh
        self._hit = hit
    ...
    def _get_path_union_by_depth(hit, depth):
        paths = []
        if depth == 0:
            paths = [(v,len(hit)) for v in list(hit.keys())]
        else:
...
def mixamo_hit(with_write=True):
    from pathlib import Path
    import os
    import sys
...
def minimixamo_semantic_cut_hit():
    from util.container import deep_dict_change_leaves
    from data.sets import DatasetMenu
    ds = DatasetMenu.order('MiniMixamoSkinned13Proj')
    h = ds._hit
    deep_dict_change_leaves(h._hit, ds._deformation.num_projections())
    new_h = HierarchicalIndexTree(h._hit, in_memory=True)
    print(new_h)
    new_h.export('./mini_mixamo_semantic_cuts_hit.pkl')
    ds.data_summary()
