...
def vertex_velocity(v_comp_t, v_ground_t):
    ...
    """
    shifted_v_comp = torch.cat((torch.zeros(v_comp_t.shape[0], 1, *v_comp_t.shape[2:]), v_comp_t))[:, -1, :, :]
    shifted_v_ground = torch.cat((torch.zeros(v_ground_t.shape[0], 1, *v_ground_t.shape[2:]), v_ground_t))[:, -1, :, :]
