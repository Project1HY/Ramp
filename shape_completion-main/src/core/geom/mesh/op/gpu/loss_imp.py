...
from geom.mesh.op.gpu.dist import batch_chamfer,batch_pdist,batch_sinkhorn

# TODO - Taken from here: https://github.com/fwilliams/fml/
# TODO - Chamfer distances based on the segmentation received from LBS weights
class SinkhornLoss(nn.Module):
    ...
    def forward(self, predicted, expected, a=None, b=None):
        ...
        M = batch_pdist(predicted, expected)

        P = batch_sinkhorn(a, b, M, self.eps, max_iters=self.max_iters, stop_thresh=self.stop_thresh)
...
class ChamferLoss(nn.Module):
    ...
    def forward(self, predicted, expected):
        ...
        """
        return batch_chamfer(predicted, expected)