...
def cdivide(a, b):
    ...
    '''
    if sps.issparse(a):
        return a.multiply(inv(b))
    elif sps.issparse(b):
        return np.asarray(a) / b.toarray()
    else:
...
def czdivide(a, b, null=0):
    ...
    '''
    if null == 0:
        return a.multiply(zinv(b)) if sps.issparse(a) else a * zinv(b)
    elif sps.issparse(b):
        b = b.toarray()
    else:
        b = np.asarray(b)
...
def point_on_line(ab, c):
    ...
    if any(len(u.shape) == 2 for u in abc):
        (a, b, c) = [np.reshape(u, (len(u), -1)) for u in abc]
    else:
        (a, b, c) = abc
    vca = a - c
    vcb = b - c
...
def point_on_segment(ab, c):
    ...
    (a, b) = ab
    abc = [np.asarray(u) for u in (a, b, c)]
    if any(len(u.shape) > 1 for u in abc):
        (a, b, c) = [np.reshape(u, (len(u), -1)) for u in abc]
    else:
        (a, b, c) = abc
    vab = b - a
...
def barycentric_to_cartesian(tri, bc):
    ...
    if tri.shape[0] == 3:
        if tri.shape[1] in [2, 3] and tri.shape[2] == n:
            ...
            raise ValueError('could not deduce triangle dimensions')
    elif tri.shape[1] == 3:
        ...
        raise ValueError('At least one dimension of triangles must be 3')
...
def point_in_triangle(tri, pt):
    ...
    if len(tri.shape) == 2 and len(pt.shape) == 1:
        if len(pt) == 2:
            ...
            dp3 = np.dot(pt - tri[2], np.cross(tri[2], tri[0] - tri[2]))
