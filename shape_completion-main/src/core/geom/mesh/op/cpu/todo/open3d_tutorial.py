...
def download_fountain_dataset():
    fountain_path = Path(__file__).parents[0] / 'test_data' / 'fountain_small'
    fountain_zip_path = Path(__file__).parents[0] / 'test_data' / 'fountain.zip'
...
def get_armadillo_mesh():
    armadillo_path = Path(__file__).parents[0] / 'test_data' / 'Armadillo.ply'
...
def get_bunny_mesh():
    bunny_path = Path(__file__).parents[0] / 'test_data' / 'Bunny.ply'
    if not os.path.exists(bunny_path):
        print("downloading bunny mesh")
        url = "http://graphics.stanford.edu/pub/3Dscanrep/bunny.tar.gz"
        urllib.request.urlretrieve(url, str(bunny_path) + ".tar.gz")
...
def get_eagle_pcd():
    path = Path(__file__) / 'test_data' / 'eagle.ply'
