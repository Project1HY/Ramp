...
#             raise ValueError(
#                 'Output reference frame of right hand side (%s) must match input reference frame of left hand side (%s)' % (
#                 other_tf.to_frame, self.from_frame))
#         pose_tf = self.pose.matrix.dot(other_tf.pose.matrix)
#         scale_tf = self.scale_ * other_tf.scale
#         return SimilarityTransform3D(pose=tfx.pose(pose_tf), scale=scale_tf, from_frame=other_tf.from_frame,
