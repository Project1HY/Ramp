...
def parser():
    ...
    # Check-pointing
    p.add_argument('--exp_name', type=str, default='Disjoint_JointExp_MiniMixamoSkinned',
...
def train_main():
    ...
    hp = nn.hp
    # Bring in data:
    ds = DatasetMenu.order(dataset_name='MiniMixamoSkinnedPyProj13',data_dir_override=r'R:\ShapeCompletion\Datasets\MiniMixamoSkinnedPyProj13')
    ldrs = ds.loaders(split=[0.8,0.1,0.1], s_nums=[10000,1000,1000], s_transform=[Center()],
               batch_size=hp.batch_size, device=hp.dev, n_channels=hp.in_channels,
               method='rand_f2jp', s_shuffle=[True] * 3, s_dynamic=[False,False,True])
